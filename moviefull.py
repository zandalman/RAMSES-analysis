...
def make_frame(nmapfile):
    ...
    for i in range(3): # loop over rows (projections)
        ...
        for j in range(3): # loop over columns (variables)
            ...
            if i==2:
                ...
                cbar.set_label([r'$\log N_{\rm DM}$ [${\rm g/cm^2}$]', r'$\log N_{\rm gas}$ [${\rm H/cm^2}$]', r'$\log N_*$ [${\rm g/cm^2}$]'][j], fontsize=14)
    ...
    axtop.axvline(x=time, color='red', lw=2)
